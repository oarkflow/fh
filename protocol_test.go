...
func TestWebSocketUpgradeAndEcho(t *testing.T) {
	app := New()
	app.Get("/ws", WebSocket(func(ws *WSConn) error {
		op, msg, err := ws.ReadMessage()
		if err != nil {
			return err
		}
		return ws.WriteMessage(op, msg)
	}))
	client := runPipeApp(t, app)
	frame := maskedFrame(WSText, []byte("hello"))
	...
	if fh[0] != 0x80|WSText || fh[1] != 5 {
		t.Fatalf("bad frame header: %x", fh)
	}
	payload := make([]byte, 5)
...
func maskedFrame(op byte, payload []byte) []byte {
	...
	if len(payload) < 126 {
		...
		b = append(b, 0x80|126, 0, 0)
		binary.BigEndian.PutUint16(b[len(b)-2:], uint16(len(payload)))
	}
	b = append(b, mask[:]...)

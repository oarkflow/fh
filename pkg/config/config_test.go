...
func TestProtocolTimeoutAndH2CConfig(t *testing.T) {
	cfg, err := LoadJSON(strings.NewReader(`{"server":{"read_header_timeout":"3s","request_body_timeout":"4s","handler_timeout":"5s","tls_handshake_timeout":"6s","http2_idle_timeout":"7s","disable_h2c":true}}`))
	if err != nil {
		t.Fatal(err)
	}
	ac, err := cfg.AppConfig()
	...
	if ac.ReadHeaderTimeout.String() != "3s" || ac.RequestBodyTimeout.String() != "4s" || ac.HandlerTimeout.String() != "5s" || ac.TLSHandshakeTimeout.String() != "6s" || ac.HTTP2IdleTimeout.String() != "7s" || !ac.DisableH2C {
	...
	if cfg.Server.RequestBodyTimeout != "8s" || !cfg.Server.DisableH2C {

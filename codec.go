...
//
// This interface intentionally stays tiny for hot-path dispatch.
...
// JSONAppender is an optional zero/low-allocation hot path for DTOs generated by
...
// sonic, go-json, jsoniter, or your own JSON adapter.
...
// it during process startup with SetJSONEngine to use a high-throughput standalone engine.
...
// direct-path behavior as jsonCodec.Marshal.
...
// applications can replace all JSON request/response work with a higher-throughput engine.
...
func (stdJSONEngine) Unmarshal(data []byte, v any) error {
	// Use the stdlib hot path. The previous Decoder+UseNumber+extra-value

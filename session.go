...
func NewSessionManager(store SessionStore, opts ...SessionOption) *SessionManager {
	...
	if !validToken([]byte(m.cookieName)) || m.maxAge < time.Second || m.path == "" || stringsContainsCTL(m.path) {
		panic(ErrInvalidSession)
	}
	probe := &Cookie{Name: m.cookieName, Value: "probe", Path: m.path, Domain: m.domain, Secure: m.secure, HttpOnly: m.httpOnly, SameSite: m.sameSite}
...
func (m *SessionManager) Begin(ctx *Ctx) (*Session, func(*Ctx) error, error) {
	raw := ctx.GetCookie(m.cookieName)
	if raw == "" {
		session := m.NewSession()
		var once sync.Once
		var completeErr error
		return session, func(responseCtx *Ctx) error {
	...
	var once sync.Once
	var completeErr error
	complete := func(responseCtx *Ctx) error {
...
func (m *SessionManager) Load(ctx *Ctx) (*Session, error) {
	raw := ctx.GetCookie(m.cookieName)
	if raw == "" {

...
func validatePlatformKernelBackend(b KernelBackend) error {
	switch b {
	case KernelBackendAuto, KernelBackendStandard, KernelBackendNative, KernelBackendEventPorts:
		return nil
	}
	return errors.New("fh: backend " + string(b) + " is not supported on Solaris/illumos")
}

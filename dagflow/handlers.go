...
func registerExampleHandlers(e *Engine) {
	e.Register("receive_email", func(rc *ExecutionContext, input any) (any, error) {
	...
	e.Register("validate_email", func(rc *ExecutionContext, input any) (any, error) {
	...
	e.Register("send_email", func(rc *ExecutionContext, input any) (any, error) {
		...
		return EmailSendResult{Request: v.Request, Sent: true, MessageID: newID("msg")}, nil
	})
	e.Register("store_message", func(rc *ExecutionContext, input any) (any, error) {
	...
	e.Register("notify_success", func(rc *ExecutionContext, input any) (any, error) {
		log.Printf("notify success task=%s", rc.TaskID)
		return map[string]any{"ok": true, "message": "email sent and stored", "data": input}, nil
	})
	e.Register("notify_failure", func(rc *ExecutionContext, input any) (any, error) {
		log.Printf("notify failure task=%s", rc.TaskID)
		return map[string]any{"ok": false, "message": "email rejected/stored", "data": input}, nil
	})
	e.Register("audit", func(rc *ExecutionContext, input any) (any, error) {
		return map[string]any{"audited": true, "at": time.Now(), "payload": input, "previous_node": rc.PreviousNode}, nil
	})
	e.Register("audit_event", func(rc *ExecutionContext, input any) (any, error) {
		return map[string]any{"audited": true, "at": time.Now(), "payload": input, "previous_node": rc.PreviousNode, "task_id": rc.TaskID}, nil
	})

	e.Register("outbox_publish", func(rc *ExecutionContext, input any) (any, error) {
		if s, ok := e.Store().(ExtendedStore); ok {
			ev := OutboxEvent{ID: newID("outbox"), TaskID: rc.TaskID, NodeID: rc.NodeID, Topic: fmt.Sprint(rc.NodeParams["topic"]), Payload: input, Status: "pending", NextRunAt: time.Now(), CreatedAt: time.Now(), UpdatedAt: time.Now()}
			if ev.Topic == "" || ev.Topic == "<nil>" {
				ev.Topic = "workflow.event"
			}
	...
	e.Register("identity", func(rc *ExecutionContext, input any) (any, error) { return input, nil })
	e.Register("uppercase", func(rc *ExecutionContext, input any) (any, error) { return strings.ToUpper(fmt.Sprint(input)), nil })
	e.Register("check_inventory", func(rc *ExecutionContext, input any) (any, error) {
		return map[string]any{"check": "inventory", "ok": true, "input": input}, nil
	})
	e.Register("check_payment", func(rc *ExecutionContext, input any) (any, error) {
		return map[string]any{"check": "payment", "ok": true, "input": input}, nil
	})
	e.Register("check_fraud", func(rc *ExecutionContext, input any) (any, error) {
		return map[string]any{"check": "fraud", "ok": true, "input": input}, nil
	})
	e.Register("approve_order", func(rc *ExecutionContext, input any) (any, error) {
		return map[string]any{"approved": true, "checks": input, "approved_at": time.Now()}, nil
	})
	e.Register("api_response", func(rc *ExecutionContext, input any) (any, error) {
		return map[string]any{"success": true, "data": input}, nil
	})
	e.Register("receive_batch", func(rc *ExecutionContext, input any) (any, error) { return toSlice(input) })
	e.Register("uppercase_one", func(rc *ExecutionContext, input any) (any, error) {
		time.Sleep(20 * time.Millisecond)
		return strings.ToUpper(fmt.Sprint(input)), nil
	})
	e.Register("finalize_batch", func(rc *ExecutionContext, input any) (any, error) {
		xs, _ := toSlice(input)
		return map[string]any{"items": input, "count": len(xs), "previous_results": rc.NodeResults}, nil
	})
	e.Register("approval_result", func(rc *ExecutionContext, input any) (any, error) {

...
interface DeviceSigningKeyRecord {
  version: 1;
...
function validClientHello(data: Uint8Array): boolean {
  ...
  if (data.byteLength < fixedLength + 2) return false;
...
async function signingKeyRecord(): Promise<DeviceSigningKeyRecord | undefined> {
  const record = await readStore<DeviceSigningKeyRecord>(KEY_STORE, SIGNING_KEY_ID);
  if (!record || record.version !== 1 || !(record.privateKey instanceof CryptoKey)) return undefined;
  const publicKey = new Uint8Array(record.publicKey);
  if (publicKey.byteLength !== 32 || record.privateKey.type !== "private" || record.privateKey.extractable) return undefined;
  if (record.privateKey.algorithm.name !== "Ed25519" || !record.privateKey.usages.includes("sign")) return undefined;
  return record;
}

export async function createDeviceKey(): Promise<{ publicKey: Uint8Array }> {
  const existing = await signingKeyRecord();
  if (existing) return { publicKey: new Uint8Array(existing.publicKey).slice() };
  ...
  if (publicKey.byteLength !== 32) throw new Error("Browser returned an invalid Ed25519 public key");
  const record: DeviceSigningKeyRecord = {
    version: 1,
...
export async function loadDevice(): Promise<StoredDevice | null> {
  const record = await readStore<DeviceIdentityRecord>(DEVICE_STORE, DEVICE_ID);
...
export async function signClientHello(data: Uint8Array): Promise<Uint8Array> {
  if (!validClientHello(data)) throw new Error("FH secure signing accepts only a versioned ClientHello transcript");
  const record = await signingKeyRecord();
  if (!record) throw new Error("FH secure device signing key is unavailable");
...
export function installSecureStorageBridge(): void {
